...
- fcac4b9: `VPick` can search a server. Pass `fetchOptions`, which receives the typed query
...
- 9ecc830: Built-in search now ignores accents as well as case, so typing "cafe" finds
...
- 83afae3: Fix: removing a chip now emits `deselect`. The chip's remove button and
