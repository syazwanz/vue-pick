...
defineOptions({ name: "VPickNative" })
...
<template>
  <div class="vpick-native-wrapper" :class="{
    ...
    <select :id="id" :name="name" class="vpick-native" :class="[
    ...
    ]" :value="String(modelValue ?? '')" :disabled="disabled || loading" :required="required"
      ...
      <template v-for="item in normalized" :key="item.label">
        <!-- Option group -->
        <optgroup v-if="isOptionGroup(item)" :label="item.label" :disabled="item.disabled"
        ...
        <!-- Flat option -->
        <option v-else :value="String(item.value)" :disabled="item.disabled" class="vpick-native-option">
          {{ item.label }}
        </option>
      </template>
    </select>
